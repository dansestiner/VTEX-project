const Methods = {
  ...
  getInstagram() {
    const endpoint = `http://sestinidocs.com.br/links/sestini/ses-ig/getMasterData.php`;
    fetch(endpoint)
      .then(res => res.json())
      .then(data => {
    ...
    data.reverse().map((item) => {
      switch (item.media_type) {
        ...
        case "CAROUSEL_ALBUM":
          ...
          break;
        case "VIDEO":
          $('#ig-images').append(`<a href='${item.permalink}' rel="noreferrer" target='_blank' title='${item.caption}'><video loop="true" class='lozad' autoplay="autoplay" muted><source src="${item.media_url}" type="video/mp4"></video></a>`)
      }
    })
