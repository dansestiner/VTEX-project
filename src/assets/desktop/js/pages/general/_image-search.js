const Methods = {
    ...
    bindHoverCamButton() {
        const imageSearchHover = $('.x-header-container__top-image-search-hover')
        $('.x-header-container__top-image-search').hover(() => {
            imageSearchHover.css('display', 'flex')
        }, () => {
            setTimeout(() => {
                if ($('.x-header-container__top-image-search:hover').length < 1) {
                    imageSearchHover.css('display', 'none')
                }
            }, 500);
        })
    },
    addImageSearchButton() {
        const template = `
        <div class='x-header-container__top-image-search'>
            <div id='cam-icon'></div>
            <div class='x-header-container__top-image-search-hover'>
                <div id='cam-icon'></div>
                <h2 class='x-header-container__top-image-search-hover-title'>Busca por imagem</h2>
                <p class='x-header-container__top-image-search-hover-text'>Agora você também pode fazer buscas no site usando fotos da sua própria galeria de imagens.</p>
                <button class='x-header-container__top-image-search-hover-button'>Usar Agora</button>
                <input type='file' id='image-search-file' style='display: none'/>
            </div>
        </div>`
        $('.x-header-container__top-search-container').append(template)
    },
    bindUploadButton() {
        $('.x-header-container__top-image-search-hover-button').click(() => $('#image-search-file').click())
    },
    bindUploadFile() {
