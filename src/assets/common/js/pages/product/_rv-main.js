...
const Methods = {
  ...
  getData() {
    ...
    Sestini.vtexCatalog
      ...
      .then((product) => {
        ...
        ProductRvData.alturareal = product._AlturaReal;
            ...
            if (product.Diferenciais) {
              product.Diferenciais.map((spec) => {
                var filter = data.filter(
                  (e) => e.description.toLowerCase() == spec.toLowerCase()
                )[0];
      ...
      .done((product) => {
        ...
        console.log(ProductRvData);
        Methods.getRelatedIds(
          ProductRvData.productId,
          ProductRvData.productResponse
        );
      })
  ...
  bindData() {
    ...
    rivets.formatters.stripHost = (item) =>
      Sestini.globalHelpers.stripHost(item);
  ...
  getRelatedIds(skuId, product) {
    ...
    fetch(urlFetch)
      ...
      .then((data) => {
        ...
        Methods.getRelatedProducts(ids, product);
      });
  },

  getRelatedProducts(ids, product) {
    ...
    const currentProductColor =
      currentProductNameArray[currentProductNameArray.length - 1];
    let currentProductSize = product["Tamanho"][0];

    const isUnavailable = $(".x-product__notifyme").css("display") == "flex";
    const currentSizeTemplate = `
      <div class="x-product__select-sizes active ${
    ...
    searchProductArray(ids).then((products) => {
      // --------------- SIZES --------------- //
      const sizes = Sestini.globalHelpers.groupObjectByValue(
        products,
        "Tamanho"
      );
      console.log("s", sizes);
      const bordo =
        sizes.Bordo != undefined
          ? sizes.Bordo.filter((item) => {
              const productNameArray = item.productName.split(" ");
              const productColor =
                productNameArray[productNameArray.length - 1];
              if (productColor == currentProductColor) return true;
            })
          : [];

      const pequeno =
        sizes.Pequeno != undefined
          ? sizes.Pequeno.filter((item) => {
              const productNameArray = item.productName.split(" ");
              const productColor =
                productNameArray[productNameArray.length - 1];
              if (productColor == currentProductColor) return true;
      ...
      const medio =
        sizes.Médio != undefined
          ? sizes.Médio.filter((item) => {
              const productNameArray = item.productName.split(" ");
              const productColor =
                productNameArray[productNameArray.length - 1];
              if (productColor == currentProductColor) return true;
      ...
      const grande =
        sizes.Grande != undefined
          ? sizes.Grande.filter((item) => {
              const productNameArray = item.productName.split(" ");
              const productColor =
                productNameArray[productNameArray.length - 1];
              if (productColor == currentProductColor) return true;
      ...
      const finalSizes = {};
      if (bordo.length > 0) {
        finalSizes["Bordo"] = bordo;
        var hasBordo = true;
      }
      if (pequeno.length > 0) {
        finalSizes["Pequeno"] = pequeno;
        var hasPequeno = true;
      }
      if (medio.length > 0) {
        finalSizes["Médio"] = medio;
        var hasMedio = true;
      }
      if (grande.length > 0) {
        finalSizes["Grande"] = grande;
      }

      console.log("final", finalSizes);
      ProductRvData.sizes = finalSizes;
      console.log("currentProductSize", currentProductSize);
      setTimeout(() => {
        switch (currentProductSize.toLowerCase()) {
          ...
          case "pequena":
            hasBordo
              ? $(".x-product__sizes-content > div:nth-child(1)").after(
                  currentSizeTemplate
                )
              : $(".x-product__sizes-content").prepend(currentSizeTemplate);
            break;
          ...
          case "média":
            if (hasBordo && hasPequeno) {
              $(".x-product__sizes-content > div:nth-child(2)").after(
                currentSizeTemplate
              );
              hasMedio &&
                $(".x-product__sizes-content > div:nth-child(4)").remove();
            } else {
              if (hasBordo || hasPequeno) {
                $(".x-product__sizes-content > div:nth-child(1)").after(
                  currentSizeTemplate
                ...
                hasMedio &&
                  $(".x-product__sizes-content > div:nth-child(3)").remove();
              } else {
                $(".x-product__sizes-content").prepend(currentSizeTemplate);
                hasMedio &&
                  $(".x-product__sizes-content > div:nth-child(2)").remove();
              }
            }
            break;
          case "grande":
      ...
      Object.keys(finalSizes).forEach((key) => {
        const availableQuantity =
          finalSizes[key][0].items[0].sellers[0].commertialOffer
            .AvailableQuantity;
      ...
      $(
        ".x-product__right > section > div.x-product__colors-wrapper > div:nth-child(1) > div > div"
      ).remove();
      $(".x-product__colors-content")
        .find(".x-product__select-color")
        .each((i, el) => {
          if ($(el).attr("data-sku") == ProductRvData.skuId) {
            $(el).addClass("is--active");
            $(".x-product__notifyme").css("display") == "flex" &&
              $(el).addClass("is--unavailable-main");
          }
        });
    });
  },
