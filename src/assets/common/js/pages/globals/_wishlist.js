const Methods = {
  ...
  getWishlist() {
    ...
    if (userData.visitorLoginState) {
      fetch(
        `https://sestinidocs.com.br/links/sestini/ecomm-api/get-wishlist.php?email=${userData.visitorContactInfo[0]}`
      )
        .then((res) => res.json())
        .then((data) => {
          ...
          Methods.setWishlistActive(
            $(".x-product-wishlist__add-btn"),
            wishlist
          );
          window.location.href.indexOf("wishlist") != -1 &&
            Methods.showShelfProducts(wishlist);
        });
    }
  },
  initWishlistMethods() {
  ...
  bindClick(e) {
    ...
    if (userData.visitorLoginState) {
      ...
      if (typeof vtexid !== "undefined") {
        vtexid.start({
          returnUrl: "/",
          forceReload: false,
        });
      }else{
        window.sessionStorage.setItem('wishlistItem', productId)
        window.sessionStorage.setItem('wishlistURL', window.location.href);
  ...
  saveProduct(email, productId, element) {
    fetch(
      `https://sestinidocs.com.br/links/sestini/ecomm-api/save-product-wishlist.php?email=${email}&productId=${productId}`
    );
    element.addClass(`is--active`);
  },
  removeProduct(email, productId, element) {
    fetch(
      `https://sestinidocs.com.br/links/sestini/ecomm-api/delete-product-wishlist.php?email=${email}&productId=${productId}`
    );
  ...
  setWishlistActive(element, wishlist) {
    element.each((i, e) => {
      wishlist.indexOf($(e).attr("data-wishlist-product-id")) != -1 &&
  ...
  showShelfProducts(wishlist) {
    if (wishlist.length >= 1) {
          ...
          const template = `
                    <article class="x-wishlist__item" >
                        <div class="x-wishlist__item-img-container">
                            <button class="x-wishlist__add-btn is--active" data-wishlist-product-id='${
                              product.productId
                            }'></button>
                            <a href='${product.link}'><img src='${
            ...
            product.productName
                            ...
                            <a href='${
                              product.link
                            }' class='product-buy-btn'>Comprar</a>
                        </div>
      ...
      $(`.x-wishlist__items`).append(
        `<h2 class='x-wishlist__empty-msg'>Sua lista de favoritos está vazia</h2>`
      );
    }
  },
  searchProductsData(wishlist, callback) {
    $.ajax({
      url:
        ...
        Methods.mountProductSearchUrl(wishlist),
      dataType: "json",
      success: (response) => {
        callback(response);
      },
