...
const Methods = {
  ...
  removeVirtusPay() {
    ...
    if (parseFloat(valueFormatted) < 150) {
      setTimeout(() => {
        ...
        $("#payment-group-VirtusPayPaymentGroup")
          .attr("onclick", "")
          .unbind("click");
      }, 1000);
    } else {
      $("#payment-group-VirtusPayPaymentGroup").click(
  ...
  setInstallmentsVirtusPay() {
    setTimeout(() => {
      ...
      fetch(
        `https://sestinidocs.com.br/links/sestini/ecomm-api/get-installments-vp.php?email=${email}&value=${formattedValue}`
      )
        .then((res) => res.json())
        .then((data) => {
          ...
          if (Object.keys(data.installments).length > 0) {
            Object.keys(data.installments).map((installment) => {
              $(".select-vp").append(
                `<option>${data.installments[installment]}</option>`
              );
            });
          } else {
            $(".select-vp").remove();
  ...
  addCheckbox() {
    $(
      "#client-profile-data > div > div.accordion-body.collapse.in > div > div > form > div > p.newsletter"
    ).prepend(`
  ...
  addAlsoBought() {
    ...
    fetch(
      `https://sestinidocs.com.br/links/sestini/ecomm-api/get-who-bought-also-bought.php?id=${searchSkuId}`
    )
      .then((res) => res.json())
      .then((data) => {
        let items = ``;
        ...
        data.map((item) => {
          ...
          let { AvailableQuantity, ListPrice, Price } =
          ...
          if (counter < 5 && AvailableQuantity > 0) {
            items += `<div class='also-bought-items-item'>
                            ...
                            ${
                              ListPrice > Price
                                ? `<p class='also-bought-items-item-de'>De: R$ ${parseFloat(
                                    ListPrice,
                                    2
                                  )
                                    .toFixed(2)
                            ...
                            <p class='also-bought-items-item-por'>Por: R$ ${parseFloat(
                              Price,
                              2
                            )
                              .toFixed(2)
  ...
  setActiveProgressBarCheckout(active) {
    $(".progress-bar-item").each((index, elem) =>
  ...
  addProgressbarCheckout() {
    $(
      "body > div.container.container-main.container-order-form > div.checkout-container.row-fluid.orderform-active > div.row-fluid.orderform-template.span12.active"
    ).prepend(`
  ...
  changeLayoutCart() {
    var buttons = $(
      "body > div.container.container-main.container-cart > div.checkout-container.row-fluid.cart-active > div.cart-template.full-cart.span12.active > div.clearfix.pull-right.cart-links.cart-links-bottom.hide"
    ).html();
    $(
      "body > div.container.container-main.container-cart > div.checkout-container.row-fluid.cart-active > div.cart-template.full-cart.span12.active > div.clearfix.pull-right.cart-links.cart-links-bottom.hide"
    ).remove();
    $(
      "body > div.container.container-main.container-cart > div.checkout-container.row-fluid.cart-active > div.cart-template.full-cart.span12.active > div.summary-template-holder > div > div.span5.totalizers.summary-totalizers.cart-totalizers.pull-right"
    ).append(buttons);

    $("#shipping-preview-container > div > h2").html(
    ...
    // Add cart title
    $("#cartLoadedDiv > div.cart").prepend(
  ...
  getProductsValue() {
    ...
    if (filteredDataLayer.length > 0) {
      let totalPrice = 0;
      filteredDataLayer[
        filteredDataLayer.length - 1
      ].ecommerce.checkout.products.map((product) => {
      ...
      fetch(`https://viacep.com.br/ws/${cep}/json/`)
        ...
        .then((data) => {
          ...
          if (
            ...
            if (limit != 199.0) var limit = 249.0;
          } else {
            var limit = 369.0;
          }
          ...
          width == 100 &&
            $(".x-product__free-shipping-bar").css("width", "94%");
        });
    }, 2000);
  },
  ...
  changeImageResolution() {
    $(window).on("load orderFormUpdated.vtex", () => {
      let changeResolution = function (index, el) {
        ...
        $(el).find("img").attr("src", imageUp);
      };

      $(
        ".body-cart .product-image a:not(.is--hd), .summary-cart-template-holder .cart .cart-items a:not(.is--hd)"
      )
        .delay()
          ...
          let advcake_content = {
            ...
            basketProducts: orderForm.items.map((item) => {
              ...
              return {
                ...
                categoryName:
                  productCategories[productCategories.length - 1][1],
              };
            }),
            orderInfo: {
  ...
  updateAddServiceText() {
    $(window).on("orderFormUpdated.vtex", () => {
      let text = $(".product-item .add-service-container .add-service.btn")
        .text()
        .replace("Adicionar", "+");
